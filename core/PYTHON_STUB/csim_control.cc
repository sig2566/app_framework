...
void CSimControl::Init(const char *cfg_path)
{
	...
	control_ptr_ = (IGL_ControlAPI*)tmp_ptr;
	ASSERT(control_ptr_);

	target_ptr_->IGetTarget(E_DEBUG_API, 0,  &tmp_ptr);
	debug_ptr_ = (IGL_DebugAPI*)tmp_ptr;
	ASSERT(debug_ptr_);
	//char *tmp_str= cfg_path;
	res = control_ptr_->IInit(reinterpret_cast<IGL_ControlCallBackAPI*>(this), cfg_path, NULL);
	ASSERT(res == E_OK);
